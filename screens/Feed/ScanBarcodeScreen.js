...
const ScanBarcodeScreen = ({ navigation }) => {
  ...
  const requestPermission = async () => {
    ...
    if (!res.granted) {
        Alert.alert('Go to Settings', 'We could not ask for camera permission! Please try granting permission in your settings app.')
    }
  }
  ...
  if (!permission.granted) {
    ...
    return (
    <View className='bg-black flex-1 items-center justify-center p-2'>
        <Text className='text-center text-lg font-bold text-white'>We need your permission to show the camera.</Text>
        <TouchableOpacity className='bg-green-900 p-4 mt-3 rounded-2xl' onPress={requestPermission}>
            <Text className='text-center text-base text-white'>Grant Permission</Text>
        </TouchableOpacity>
    </View>
    );
  }
  ...
  return (
    <View className='flex-1 justify-center bg-black'>
      <CameraView barcodeScannerSettings={{
        ...
        }} onBarcodeScanned={handleScannedBarcode} className='flex-1' facing={facing}>
        <View className='flex-1 flex-row bg-transparent m-16'>
        <TouchableOpacity className='flex-1 self-end items-center' onPress={() => navigation.goBack()}>
